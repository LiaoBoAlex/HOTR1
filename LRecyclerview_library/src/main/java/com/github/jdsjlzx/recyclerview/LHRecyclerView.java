...
import com.github.jdsjlzx.view.HLoadingFooter;
...
public class LHRecyclerView extends RecyclerView {
    ...
    private boolean mRefreshing = false;//是否正在右拉刷新
    ...
    public LHRecyclerView(Context context) {
        this(context, null);
    }

    public LHRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LHRecyclerView(Context context, AttributeSet attrs, int defStyle) {
            ...
            if (adapter instanceof LRecyclerViewAdapter) {
                ...
                if (lRecyclerViewAdapter.getInnerAdapter() != null && mEmptyView != null) {
                    ...
                    if (count == 0) {
                        ...
                        LHRecyclerView.this.setVisibility(View.GONE);
                    } else {
                        mEmptyView.setVisibility(View.GONE);
                        LHRecyclerView.this.setVisibility(View.VISIBLE);
                    }
                }
            } else {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                final float deltaX = (ev.getRawX() - mLastX) / DRAG_RATE;
                mLastX = ev.getRawX();
                sumOffSet += deltaX;
                if (isOnTop() && mPullRefreshEnabled && !mRefreshing && (appbarState == AppBarStateChangeListener.State.EXPANDED)) {
                    mRefreshHeader.onMove(deltaX, sumOffSet);
                    if (mRefreshHeader.getVisibleWidth() > 0) {
                        return false;
                    }
                }
    ...
    public void setLoadMoreFooter(ILoadMoreFooter loadMoreFooter) {
        ...
        if (layoutParams != null) {
            ...
            mFootView.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT));
        }
    }
    ...
    public void refresh() {
        ...
        if (mPullRefreshEnabled && mRefreshListener != null) {
            ...
            int offSet = mRefreshHeader.getHeaderView().getMeasuredWidth();
